...
\subsection{Role of the Human Pilot}
...
Our assumptions are that the human pilot can sense (i) $y_{pilot} \subseteq y$, (ii) $r$, (iii) $u$, (iv) $\delta_a$, and (v) $x_{pilot} \subseteq x_p$, where $y_{pilot}$ is a subset of vehicle sensor measurements available to the pilot, and $x_{pilot}$ is a subset of vehicle states which the pilot is able to sense independent of the vehicle's sensors. The hypothesis is that (i)-(iii) are accessible through cockpit displays, (iv) is accessible through cockpit control interfaces, and (v) is accessible through the pilot's visual and vestibular sensing. In addition to changes in closed-loop behavior in both anomalies introduced in Section \ref{sec:problem}, the physical actuator fault would change the relationship between $u$ and $\delta_a$, while the time-delayed sensor measurements would change the relationship between $x_{pilot}$ and $y_{pilot}$. We hypothesize that incorporating the human pilot in detection and decision-making tasks may allow the autopilot to retain autonomous control in situations where reliance on autonomous anomaly detection and diagnosis is impractical. 
...
\subsection{MRAC-Based Adaptive Autopilot}
...
The plant to be controlled is an \textit{n}th-order linear dynamical system given by
\begin{equation}
	\dot{x}_p(t) = A_p x_p(t) + B_p u(t)
	\label{eqn:plant}
\end{equation}
\noindent where $A_p \in \mathbb{R}^{n \times n}$ and $B_p \in \mathbb{R}^{n \times 1}$ are an unknown matrix and vector, respectively, and $u(t)$ is a scalar input. The feedforward/feedback control law has the form
\begin{equation}
...
\subsection{The Overall Shared Controller}
\begin{figure}[h]
	\centering
